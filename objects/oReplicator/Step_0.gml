...
if Phenotype[1] == 1{
	if (changeableValues[2] >= Genotype[1,0]) && (changeableValues[2] >= (Genotype[1,2] * Genotype[1,1])){
		repeat(Genotype[1,1]){
			...
			child.Genotype[0,0] = sGeneMutator(Genotype[0,0],1,1,2,child.id,"massType");
			child.Genotype[0,1] = sGeneMutator(Genotype[0,0],0,0,2,child.id,"Eats");
			child.Genotype[0,2] = sGeneMutator(Genotype[0,0],0,100,2,child.id,"metbolismRate");
